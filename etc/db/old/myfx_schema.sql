...
create table t_account (
   ...
   lastreportedbalance decimal(10,2) comment 'letzter gemeldeter Kontostand',
   lastupdate datetime comment 'Zeitpunkt des letzten History-Updates (read-only)',
...
create table t_news (
   ...
   version timestamp not null default current_timestamp on update current_timestamp,
   created datetime comment '(read-only)',
   time datetime not null comment 'timezone: GMT',
   currency enum('AUD','CAD','CHF','EUR','GBP','JPY','NZD','USD') not null,
   impact enum('high','medium','low','holiday') not null,
...
begin
   ...
   if (new.lastupdate is not null) then call ERROR_ACCOUNT_LASTUPDATE_IS_READONLY(); end if;
...
begin
   ...
   if (       old.ticket         !=       new.ticket         ) then call ERROR_TRANSACTION_TICKET_IS_READ_ONLY();      end if;
   if (       old.units          !=       new.units          ) then call ERROR_TRANSACTION_UNITS_IS_READ_ONLY();       end if;
   if (ifNull(old.symbol     ,'')!=ifNull(new.symbol     ,'')) then call ERROR_TRANSACTION_SYMBOL_IS_READ_ONLY();      end if;
   if (       old.opentime       !=       new.opentime       ) then call ERROR_TRANSACTION_OPENTIME_IS_READ_ONLY();    end if;
   if (ifNull(old.openprice  ,'')!=ifNull(new.openprice  ,'')) then call ERROR_TRANSACTION_OPENPRICE_IS_READ_ONLY();   end if;
   if (       old.closetime      !=       new.closetime      ) then call ERROR_TRANSACTION_CLOSETIME_IS_READ_ONLY();   end if;
   if (ifNull(old.closeprice ,'')!=ifNull(new.closeprice ,'')) then call ERROR_TRANSACTION_CLOSEPRICE_IS_READ_ONLY();  end if;
   if (       old.commission     !=       new.commission     ) then call ERROR_TRANSACTION_COMMISSION_IS_READ_ONLY();  end if;
   if (       old.swap           !=       new.swap           ) then call ERROR_TRANSACTION_SWAP_IS_READ_ONLY();        end if;
   if (       old.netprofit      !=       new.netprofit      ) then call ERROR_TRANSACTION_NETPROFIT_IS_READ_ONLY();   end if;
   ...
   if (old.type != new.type) then
      if (old.type not in ('transfer','vendor') || new.type not in ('transfer','vendor')) then
