...
class UploadAccountHistoryActionForm extends ActionForm {
    ...
    protected function populate(Request $request) {
        if ($request->isPost()) {
            ...
            // MQL-API
                ...
                // Daten in temporaere Datei schreiben und $_FILES-Array emulieren
    ...
    public function validate() {
        ...
        foreach ($lines as $i => &$line) {
            ...
            if (preg_match('/^\[(\w+)\]/i', $line, $matches)) {   // Abschnittsnamen analysieren
                ...
                $section = null;                                   // unbekannte Abschnitte und mehrfache Vorkommen gueltiger Abschnitte ueberspringen
            }
            if (!$section)                                        // alle Zeilen außerhalb gueltiger Abschnitte ueberspringen
                continue;
            ...
            if ($section == 'data') {
                ...
                if ($type==OP_BALANCE || $type==OP_CREDIT) { // fuer Balance und Credit-Werte nichtzutreffende Felder auf NULL setzen
                    $symbol      = null;
                    $openPrice   = null;
                    $closePrice  = null;
                    $magicNumber = null;
    ...
    public function __destruct() {
        ...
        try {
            // tmp. Dateien manuell loeschen, da $FILES-Array u.U. emuliert sein kann
